...
var configure = function (options) {
    ...
    if (options.token !== undefined) {
        var setAuth = function (val) {
            if (!val)
                delete exports.Opt.headers["Authorization"];
            else
                exports.Opt.headers["Authorization"] = val.startsWith("Bearer ") ? val : "Bearer ".concat(val);
        };
        if (!options.token) {
            delete exports.Opt.headers["Authorization"];
        }
            ...
            try {
                var result = options.token();
                if (result instanceof Promise) {
                    result.then(setAuth).catch(function (err) { return console.warn("[doptime] Async token failed:", err); });
                }
                else {
                    setAuth(result);
                }
