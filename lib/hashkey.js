...
var hashKey = /** @class */ (function () {
    function hashKey(key, shemaToCheck) {
        if (shemaToCheck === void 0) { shemaToCheck = null; }
        var _this = this;
        this.key = key;
        this.shemaToCheck = shemaToCheck;
        this.checkShema = function (data) {
            if (_this.shemaToCheck == null)
                return true;
            var typeofdata = typeof data;
            //single value shema
            if (typeofdata != "object" && typeofdata == _this.shemaToCheck)
                return true;
            //case of object shema
            for (var field in _this.shemaToCheck) {
                var v = _this.shemaToCheck[field];
                if (typeof data[field] != v)
                    return false;
            }
            return true;
        };
        ...
        this.hset = function (Field, data, opt) {
            if (Field === void 0) { Field = ""; }
            if (opt === void 0) { opt = Option_1.Option; }
            if (!_this.checkShema(data))
                return Promise.reject("data not match shema");
            (0, http_1.default)(opt).put("".concat(opt.urlbase, "/HSET-!").concat(_this.key).concat(opt.paramString(), "?F=").concat(encodeURIComponent(Field)), data);
        };
        ...
        if (typeof shemaToCheck == "object")
            for (var field in this.shemaToCheck) {
                var v = this.shemaToCheck[field];
                this.shemaToCheck[field] = typeof v;
            }
        else
            this.shemaToCheck = typeof shemaToCheck;
    }
    return hashKey;
}());
