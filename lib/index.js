...
var setDefaultSUToken = function (sutoken) {
    if (!!sutoken)
        exports.Option.withUrlField("su", sutoken);
    else
...
//if withscores is true, return [member, score, member, score, ...]
//if withscores is false, return [member, member, ...]
var zRevRangeByScore = function (Key, Max, Min, WITHSCORES, Offset, Count, opt) {
    if (WITHSCORES === void 0) { WITHSCORES = true; }
    if (Offset === void 0) { Offset = 0; }
    if (Count === void 0) { Count = 4096; }
    if (opt === void 0) { opt = exports.Option; }
    return Req(opt).get("".concat(opt.Urlbase || urlbase, "/ZREVRANGEBYSCORE-!").concat(Key, "-!").concat(opt.paramString(), "?Min=").concat(Min, "&Max=").concat(Max, "&WITHSCORES=").concat(WITHSCORES, "&Offset=").concat(Offset, "&Count=").concat(Count));
};
